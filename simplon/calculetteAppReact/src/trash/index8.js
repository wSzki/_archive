...
class Calc extends React.Component {
  ...
  handleClick(i) {
    ...
    else if (i === ".") {if ((displayState !== "") && (!(decimalState))) { displayState += i; setDecimalStateTrue(); setDisplayState() }}
    ...
    // If i is an operator (+ - * =) and is not an empty string, then :
      ...
      if (i === "="){displayState = eval(temp + i + displayState) ; setDisplayState(); resetState()}

      else {
      temp = displayState

      if ((!(isNumber)) && (i !== ".")) { temp = temp.slice(0, -1) + i }
      else {temp += i;}
