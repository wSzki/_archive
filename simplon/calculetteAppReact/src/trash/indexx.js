...
class Calc extends React.Component {
  ...
  handleClick(i) {
    ...
    let setStateActive = (a) => this.setState({ active: a })
    ...
    else if (isNumber) { if (init) { initScreen(); } displayState += i; }

    // Checking if " . " and if unique, then append to string
    else if ((i === ".") && (displayState !== "") && (!(decimalState))) { displayState += i; setDecimalStateTrue(); }
    ...
    // If i is an operator (+ - * =) and is not an empty string, then :
      ...
      if ((isActive) && (!(isNumber)) && (i !== ".")) { temp = temp.slice(0, -1) + i }
      else { temp = displayState + i; setStateActive(true) }


      this.setState({ temp: temp })
      displayState = eval(temp + displayState)
